...
const executeSendRequest = async ({ message, author }) => {
  ...
  try {
    ...
    //
    const response = await request.post('http://dev.pibrain.ngrok.io/predict', { form : {sentence: message} })

    return { err: false, response: `Sure, let me handle that for you.`, data: response }
  } catch(err) {
    console.error(err)
