...
module.exports = function (sequelize, DataTypes) {
  const attributes = {
    ...
    password: {
      ...
      validate: {
        ...
        isSufficientlyComplex: function isSufficientlyComplex(val) {
          var arrayPass = new Array(val);
          let passesSymCheck = arrayPass.some(chr => {
            return chr.match(/[ !"#\$%&'\(\)\?\*\+\,\-\./\:;\<\=\>?@\[\\\]\^_`\{\|\}~]/);
          });
          let passesNumericCheck = arrayPass.some(chr => {
            return chr.match(/\d/);
          });
          let passesAlphaCheck = arrayPass.some(chr => {
            return chr.match(/[a-zA-Z]/);
          });
  ...
  const hashPassword = (user, options) => {
    ...
    return _argon2.default.hash(user.password, { type: _argon2.default.argon2i }).then(hash => {
